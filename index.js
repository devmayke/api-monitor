...
let transporter = nodemailer.createTransport({
  ...
  auth: {
    ...
    pass: "Tua senha aqui"//////tua senha aqui
  }
})
function enviarEmail(server) {
  transporter.sendMail({
    from: `${'Mayke'} <devmayke@gmail.com>`,////Teu email aqui
    to: "incognator@gmail.com",
    subject: `Server ${server} Crashou`,
    html: `**${server}** Hora que parou de funcionar -   ${new Date().getHours()}: ${new Date().getMinutes()}:${new Date().getSeconds()}`
  }).then((sucesso) => {
    console.log(sucesso)
    clearInterval(time)
  }).catch((err) => {
